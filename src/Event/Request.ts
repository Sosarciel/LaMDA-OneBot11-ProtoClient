...
/**加好友请求快速操作 */
export class FriendRequestQO extends QuickOperationTool<FriendRequestQuickOperation> {
    /**快速回复
     * @param approve - 是否同意请求
     * @param remark - 添加后的好友备注（仅在同意时有效）
     */
    approve(approve: boolean, remark?: string) {
        return this.qo({
            approve,
            remark,
        });
...
/**加群请求／邀请快速操作 */
export class GroupRequestQO extends QuickOperationTool<GroupRequestQuickOperation> {
    /**快速回复
     * @param approve - 是否同意请求／邀请
     * @param reason - 拒绝理由（仅在拒绝时有效）
     */
    approve(approve: boolean, reason?: string) {
        return this.qo({
            approve,
            reason,
        });
