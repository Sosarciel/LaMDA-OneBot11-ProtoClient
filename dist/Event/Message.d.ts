...
/**群快速操作 */
export declare class GroupMessageQO extends QuickOperationTool<GroupMessageQuickOperation> {
    /**快速回复
     * @param reply       - 消息内容
     * @param auto_escape - 消息内容是否作为纯文本发送（即不解析 CQ 码），只在 reply 字段是字符串时有效
     * @param at_sender   - 是否要在回复开头 at 发送者（自动添加），发送者是匿名用户时无效
     */
    reply(reply: Message, auto_escape: boolean, at_sender: boolean): boolean;
    /**撤回该条消息 */
    delete(): boolean;
    /**把发送者踢出群组 */
    kick(): boolean;
     ...
     */
    ban(ban_duration: number): boolean;
}
/**群消息事件
 * @param data - 事件数据
 * @param qo   - 快速操作
...
/**私聊快速操作 */
export declare class PrivateMessageQO extends QuickOperationTool<PrivateMessageQuickOperation> {
    /**快速回复
     * @param reply       - 消息内容
     * @param auto_escape - 消息内容是否作为纯文本发送（即不解析 CQ 码），只在 reply 字段是字符串时有效
     */
    reply(reply: string, auto_escape: boolean): boolean;
}
/**私聊消息事件
 * @param data - 事件数据
 * @param qo   - 快速操作
